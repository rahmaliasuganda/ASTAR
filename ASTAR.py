...
def a_star_with_path(start):
    start_time = time.time()
    open_set = []
    heapq.heappush(open_set, (misplaced_tiles(start), 0, start))
    visited = set()
    ...
    while open_set:
        ...
        if current == GOAL_STATE:
            ...
            path.reverse()
            return time.time() - start_time, nodes_explored, g, path
...
def run_astar_experiments(n=5):
    ...
    for i in range(1, n+1):
        ...
        time_taken, nodes, path_len, path = a_star_with_path(start_state)
